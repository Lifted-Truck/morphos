...
- [x] **Phase 5 (partial)** — Field-model extensions:
...
- [ ] **Phase 6** — Modulation: mod matrix, MIDI sources, Morphon state sources, MPE.
  ...
  - **Path Objects as modulation sources** — once Path Objects exist (Phase 5 remainder), they should expose progress-along-curve, tangent-angle, and current-XY as mod-matrix sources, so any parameter can be driven by a Morphon's (or an Emitter's) position along a user-drawn curve.
