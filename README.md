...
- [ ] **Phase 6** — Modulation: mod matrix, MIDI sources, Morphon state sources, MPE.
  ...
  - **Bipolar / unipolar toggle** per modulator (random sources included). Designed, ready to build — touches only the Mod tab + mod eval:
    - `ModMatrix.h`: add `bool bipolar = true;` to `ModConnection` (field-by-field assignment; default preserves current behaviour).
    - Eval math (`evaluateModMatrix` in PhysicsEngine.cpp): `offset = c.bipolar ? (src-0.5f)*2.0f*c.depth*perDestSwing(dst) : src*c.depth*perDestSwing(dst);` — unipolar: src=0 sits at base, depth sign sets direction.
    - `PhysicsState.h`: append `SetModConnectionPolarity` to the Mod-matrix enum group (snapshot already copies whole `ModConnection` structs). Handle in drainEditCommands (`bipolar = e.x >= 0.5f`); set `bipolar = true` in the AddModConnection reset.
    - UI: `std::array<juce::TextButton, MAX_MOD_CONNECTIONS> modPolarityBtns_;` (after `modRemoveBtns_`), re-parent into `panelContentMod_`, style/wire in `setupModMatrix`, lay out on the depth-slider row in `layoutModTabContent` (shrink depth slider ~40px), sync text/state in `updateModTab`. Label "BI"/"UNI" (wording tunable later). No dclickDefaults entry — it's a button.
    - Patch: `node.setProperty("bipolar", c.bipolar)` write; `getProperty("bipolar", true)` read (legacy-safe = bipolar).
  - **Morphon velocity as a mod source** — a Morphon's speed/velocity drives modulation, but **only routable to per-Morphon destinations** (that Morphon's own timbre/pan/amp/etc.), not shared object/global params. Velocity is per-particle, so it needs per-Morphon mod evaluation in `blendAnchorsGranular`/integrate, distinct from the current global single-value mod write — the matrix UI should hide/disable non-per-Morphon destinations when this source is selected.
  - **MPE** — per-note expression (MIDI 2.x / MPE: per-note pitch bend, pressure, slide/CC74) as mod sources keyed to the spawning note, routed to that note's Morphon(s). Shares the per-Morphon mod-evaluation machinery the velocity source needs; spawn-time channels must map to the Morphon(s) launched for that note (note the Emitter Morphon-count fan-out — one note can own N Morphons).
...
- [ ] **Phase 10** — Product:
  ...
  - **Bounding-box multi-select** — click-and-drag on empty canvas draws a rubber-band rectangle; on release every object whose centre lies inside is added to a selection set. When the selection is homogeneous (all the same type), the panel sliders edit every member at once — drag the Radius slider and every selected Effect Zone resizes together. When it's mixed, the panel collapses to the operations that make sense across types (move-all, delete-all, copy-all). Translate-by-dragging applies the same Δx, Δy to every member.
...
- **macOS: Ableton computer-MIDI keyboard stops after clicking in the plugin UI** — most visible after switching an anchor's source engine, but it happens on any click in the plugin's floating window. On macOS, clicking the plugin window makes it the **key window**, so Live's computer-MIDI keyboard no longer plays the armed track until you click back into Live's main window. Tried and reverted: stopping combos/buttons from grabbing keyboard focus, and `unfocusAllComponents()` after the source switch — neither helped, which points to OS window-key level rather than JUCE component focus. Real fixes carry trade-offs (e.g. drop the editor's keyboard focus entirely and move Delete/Esc/Copy/Paste off the keyboard, or forward keystrokes to the host). Deferred to the refinement phase; minor, only affects the *computer*-keyboard MIDI, not hardware/clip MIDI.
